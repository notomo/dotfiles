...
class Source(Base):
    ...
    def gather_candidates(self, context):
        ...
        def create(directory: str):
            return {"word": directory, "action__path": os.path.join(path, directory)}
        ...
        directories = (
            create(d) for d in os.listdir(path) if os.path.isdir(os.path.join(path, d))
        )
        ...
        return [d for d in directories if d["word"] == filetype]
