...
class Source(Base):
    ...
    def gather_candidates(self, context):
        ...
        def create(line):
            ...
            return {
                ...
                "action__is_receiver": (
                    line["full"].replace("func ", "").replace("type ", "")[0] is "("
                ),
            }

        exports = filter(
            lambda x: x["ident"][0].isupper() and x["filename"].find("_test.go") == -1,
            result,
        )
    ...
    def _execute(self, path):
        command = ["motion", "-mode", "decls", "-include", "func,type", "-dir", path]
    ...
    def _execute_project(self, top_path):
        ...
        process = subprocess.Popen(
            ["go", "list", "-f", "{{.Dir}}", package_path], stdout=subprocess.PIPE
        )
