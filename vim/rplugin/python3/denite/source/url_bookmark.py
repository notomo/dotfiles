    ...
    def gather_candidates(self, context):
        ...
        def new_file(path: str):
            with open(path, "a") as f:
                lines = ["sample\thttps://github.com/notomo/dotfiles"]
                f.writelines(lines)
        ...
        def gather(path: str):
            ...
            urls.extend(
                [create(line, path, i) for i, line in enumerate(url_file, start=1)]
            )
            url_file.close()
            return urls

        path = os.path.expanduser("~/.local/.denite_url_bookmark")
