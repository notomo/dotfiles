...
vim.keymap.set({ "x", "o" }, "ig", [[il]])
vim.keymap.set({ "x", "o" }, "ae", [[al]])
vim.keymap.set("o", "gp", [[])]])
...
vim.keymap.set({ "n", "x" }, "[yank]s", function()
  local git_root = vim.fs.root(0, ".git") or "."
  local full_path = vim.fs.normalize(vim.fn.expand("%:p"))
  local relative_path = vim.fs.relpath(git_root, full_path)
...
vim.keymap.set("n", "[edit]n", function()
  vim.cmd.normal({ args = { "*N" }, bang = true })
  require("notomo.lib.replace").replace_next()
end)
vim.keymap.set("x", "[edit]n", function()
  vim.fn.setreg("/", require("notomo.lib.replace").search_pattern())
  vim.cmd.normal({ args = { "nN" }, bang = true })
  require("notomo.lib.replace").replace_next()
end)
vim.keymap.set("x", "[edit]d", function()
  vim.fn.setreg("/", require("notomo.lib.replace").search_pattern())
  vim.cmd.normal({ args = { "nN" }, bang = true })
  vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes([["_cgn]], true, false, true), "n", false)
end)

vim.keymap.set({ "n", "x" }, "sa", function()
  return require("notomo.lib.surround").add_operator()
end, { expr = true })
vim.keymap.set({ "n" }, "sd", function()
  return "v<Cmd>lua require('curstr').execute('vim/surrounded/around')<CR>gv"
    .. require("notomo.lib.surround").delete_operator()
end, { expr = true })
vim.keymap.set({ "x" }, "sd", function()
  return require("notomo.lib.surround").delete_operator()
end, { expr = true })
vim.keymap.set({ "n" }, "sr", function()
  return "v<Cmd>lua require('curstr').execute('vim/surrounded/around')<CR>gv"
    .. require("notomo.lib.surround").replace_operator()
end, { expr = true })
vim.keymap.set({ "x" }, "sr", function()
  return require("notomo.lib.surround").replace_operator()
end, { expr = true })

local boundary = [=[\v\W\zs\w|_\zs[A-Za-z0-9]|[a-z0-9]\zs\u|\u\zs\u\l]=]
vim.keymap.set({ "n", "x", "o" }, "<Leader>w", function()
  require("notomo.lib.move").w({ pattern = boundary })
end)
vim.keymap.set({ "n", "x", "o" }, "<Leader>b", function()
  require("notomo.lib.move").b({ pattern = boundary })
end)
vim.keymap.set({ "n", "x", "o" }, "<Leader>e", function()
  require("notomo.lib.move").e({ pattern = [[\v\u\l+\d*|\u+\ze\u\l|\u+\d*|\l+\d*]] })
end)
...
vim.keymap.set({ "n", "x", "o" }, "e", function()
  require("notomo.lib.move").e({ pattern = [[\v[a-zA-Z0-9_]+|(\k&[^a-zA-Z0-9_])+]] })
end)
