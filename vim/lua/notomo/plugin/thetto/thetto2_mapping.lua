...
vim.keymap.set("n", "[finder]<CR>", function()
  require("thetto2").resume()
end)
vim.keymap.set("n", "[finder]n", function()
  require("thetto2").resume({
    item_cursor_factory = require("thetto2.util.item_cursor").offset(1),
...
local source_specific = {
  ["git/log"] = function(list_bufnr)
    vim.keymap.set("n", "yr", function()
      require("notomo.lib.edit").yank(require("thetto2").get()[1].commit_hash)
    end, { buffer = list_bufnr })
    vim.keymap.set("n", "yf", function()
      local short_commit_hash = require("thetto2").get()[1].commit_hash
      local commit_hash = vim.fn.systemlist({ "git", "rev-parse", short_commit_hash })[1]
      require("notomo.lib.edit").yank(commit_hash)
    end, { buffer = list_bufnr })
    vim.keymap.set("n", "yR", function()
      require("notomo.lib.github").yank_revision_with_repo(require("thetto2").get()[1].commit_hash)
    end, { buffer = list_bufnr })
    vim.keymap.set("n", "ym", function()
      require("notomo.lib.git").yank_commit_message(require("thetto2").get()[1].commit_hash)
    end, { buffer = list_bufnr })
    vim.keymap.set("n", "yu", function()
      require("notomo.lib.github").yank_commit_url(require("thetto2").get()[1].commit_hash)
    end, { buffer = list_bufnr })
    ...
    local action_without_message = function(action_name, opts)
      return function()
        ...
        local filtered = vim
          ...
          :totable()
        local item_action_groups = require("thetto2.util.action").grouping(filtered, {
          action_name = action_name,
          actions = metadata.actions,
        })
        require("thetto2").execute(item_action_groups, opts)
...
vim.keymap.set("n", "[finder]r", function()
  require("thetto2").start(require("thetto2.util.source").by_name("file/directory/recursive", {
    cwd = require("thetto2.util.cwd").project(),
  }))
end)
...
vim.keymap.set("n", "<Space>usg", function()
  require("thetto2").start(require("thetto2.util.source").by_name("file/recursive", {
    cwd = require("thetto2.util.cwd").project(),
    opts = ls_opts,
  }))
end)
...
vim.keymap.set("n", "[keyword]n", function()
  ...
  require("thetto2").start(require("thetto2.util.source").by_name("vim/lsp/references", {
...
vim.keymap.set("n", "<Space>qn", function()
  ...
  require("thetto2").start(
    require("thetto2.util.source").by_name("vim/diagnostic", {
      actions = {
        default_action = "open",
      },
      can_resume = false,
  ...
  require("thetto2").start(
    require("thetto2.util.source").by_name("vim/diagnostic", {
      ...
      modify_pipeline = require("thetto2.util.pipeline").append({
        require("thetto2.util.sorter").field_by_name("row", true),
      }),
    }),
    {
      item_cursor_factory = require("thetto2.util.item_cursor").search(function(item)
...
vim.keymap.set("n", "[git]xs", function()
  require("thetto2").start(require("thetto2.util.source").by_name("git/stash"), {
    consumer_factory = require("thetto2.util.consumer").immediate({ action_name = "create" }),
  })
end)

vim.keymap.set("n", "[finder]ga", function()
  require("thetto2").start(require("thetto2.util.source").by_name("git/branch", {
    modify_pipeline = require("thetto2.util.pipeline").append({
...
vim.keymap.set("n", "[git]s", function()
  require("thetto2").start(require("thetto2.util.source").by_name("git/status"), {
    item_cursor_factory = require("thetto2.util.item_cursor").search(function(item)
...
vim.keymap.set("n", "[git]B", function()
  ...
  require("thetto2").start(
    require("thetto2.util.source").by_name("git/blame", {
      consumer_opts = {
        ui = {
          insert = false,
