...
vim.keymap.set("n", "[exec]bm", function()
  require("cmdhndlr").build({ name = "make/make", runner_opts = { target = "" } })
end)
...
vim.api.nvim_create_autocmd({ "FileType" }, {
  ...
  pattern = { "cmdhndlr" },
  callback = function()
    vim.keymap.set("n", "[file]rl", [[<Cmd>lua require("cmdhndlr").retry()<CR>]], { buf = 0 })
  end,
})
