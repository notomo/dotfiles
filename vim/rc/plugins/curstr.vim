...
call curstr#custom#source_option('print_js', 'patterns', [['\v^(\s*)(let|const)?\s+([^=[:space:],]*).*$', '\1console.log(\3)']])
...
call curstr#custom#source_option('print_ts', 'patterns', [['\v^(\s*)(let|const)?\s+([^=[:space:],:]*).*$', '\1console.log(\3)']])
