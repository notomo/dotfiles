...
call curstr#custom#source_option('print_python', 'patterns', [['\v^\s*([^=[:space:],]*).*$', 'print(\1)']])
...
call curstr#custom#source_option('print_js', 'patterns', [['\v^\s*(let|const)?\s+([^=[:space:],]*).*$', 'console.log(\2)']])
...
call curstr#custom#source_option('print_ts', 'patterns', [['\v^\s*(let|const)?\s+([^=[:space:],:]*).*$', 'console.log(\2)']])
...
call curstr#custom#source_option('print_rust', 'patterns', [['\v^\s*let\s+(mut\s+)?([^=[:space:],:]*).*$', 'println!("{:?}", \2);']])
