...
class Notomo(object):
    ...
    def run_http_server_and_open(self, args):
        ...
        open_current = args[0]
        port = self._vim.vars.get("local#var#port", random.randint(49152, 65535))

        if "local#var#document_root" in self._vim.vars and not open_current:
            document_root = self._vim.call(
                "expand", self._vim.vars["local#var#document_root"]
            )
            path = ""
        else:
            document_root = self._vim.call("getcwd")
            path = self._vim.call("expand", "%:p")
            path = self._vim.call("expand", "%") if os.path.isfile(path) else ""
